...
class SslCommerzHallNotification extends AbstractSslCommerz
{
    protected $data = [];
    protected $config = [];
...
//        $this->setStorePassword($this->config['apiCredentials']['store_password']);
        $this->setStoreId(session()->get('ssl_store_id'));
        $this->setStorePassword(session()->get('ssl_store_password'));
    }
...
//                    }
    ...
    {
        $this->successUrl = url('/') . $this->config['hall_success_url'];
    ...
    {
        $this->failedUrl = url('/') . $this->config['hall_failed_url'];
    ...
    {
        $this->cancelUrl = url('/') . $this->config['hall_cancel_url'];
    }
    ...
    public function setRequiredInfo(array $info)
    {
        ...
        $this->data['hall_success_url'] = $this->getSuccessUrl(); // string (255)	Mandatory - It is the callback URL of your website where user will redirect after successful payment (Length: 255)
        ...
        $this->data['hall_cancel_url'] = $this->getCancelUrl(); // string (255)	Mandatory - It is the callback URL of your website where user will redirect if user canceled the transaction (Length: 255)
         ...
         * */
        $this->data['hall_ipn_url'] = (isset($info['hall_ipn_url'])) ? $info['hall_ipn_url'] : null;
